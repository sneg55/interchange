"""Geometry for cross-publisher work zone matching. Section 6.6.

Defines the distance measure the reconciler gates on. The choice matters and is
not arbitrary:

  - Live feeds use LineString and MultiPoint, and MultiPoint carries both single
    points (New York DOT, NJIT) and two-point segments (Iowa DOT, Mississippi
    DOT). Neither type can be assumed.
  - Distance between two features is the MINIMUM distance between their
    ...
    for long LineStrings: two publishers describing the same kilometre of roadway
    with differently clipped extents have centroids far apart while the zones
    plainly coincide.
  - Coordinates project to local metres (equirectangular about a shared standard
    parallel) before segment math. Under a few kilometres the error against the
    true geodesic is below a tenth of a percent, immaterial at a 150 m threshold.

Ported from `scripts/wzdx/geometry.py`, which is disclosed as prior-art tooling
(spec 16) and carries a regression test per defect that actually shipped. The
comments naming those defects are kept deliberately: every one of them passed
review before it was caught by a measurement.
"""
...
def vertices(feature: dict) -> list[Vertex]:
...
def to_metres(lon: float, lat: float, lat0: float | None = None) -> Vertex:
...
def unwrap(lons: list[float], reference: float) -> list[float]:
...
def _projected(verts: list[Vertex], lat0: float, reference_lon: float) -> list[Vertex]:
    lons = unwrap([v[0] for v in verts], reference_lon)
    return [to_metres(lon, v[1], lat0) for lon, v in zip(lons, verts, strict=True)]


def _point_seg_dist(p: Vertex, a: Vertex, b: Vertex) -> float:
    px, py = p
    ax, ay = a
    bx, by = b
    dx, dy = bx - ax, by - ay
...
def _seg_seg_dist(a: Vertex, b: Vertex, c: Vertex, d: Vertex) -> float:
...
def min_distance_m(va: list[Vertex], vb: list[Vertex]) -> float | None:
...
def length_m(verts: list[Vertex]) -> float:
    if len(verts) < 2:
        return 0.0
...
def densify(verts: list[Vertex], step: float = 25.0) -> list[Vertex]:
...
def coverage(va: list[Vertex], vb: list[Vertex], threshold: float) -> float:
...
def symmetric_coverage(va: list[Vertex], vb: list[Vertex], threshold: float) -> float:
...
def spatially_matches(
    va: list[Vertex], vb: list[Vertex], threshold: float
